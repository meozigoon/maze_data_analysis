...
plt.plot(df_mean['bfs mean'] + df_mean['dfs2 mean'], df_mean['bfs mean'] - df_mean['dfs2 mean'], label='BFS → DFS', marker='o')
plt.plot(df_mean['dfs mean'] + df_mean['bfs2 mean'], df_mean['dfs mean'] - df_mean['bfs2 mean'], label='DFS → BFS', marker='x')
plt.xlabel('Average Time Sum (ms)')
plt.ylabel('Average Time Difference (ms)')
